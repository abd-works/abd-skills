...
- **Package `README.md`** (at `skills/<dir>/README.md` or `agents/<dir>/README.md`)
  — human- or AI-authored card and detail copy. Convention:
...
1. **Regenerate on command.** From the **agilebydesign-skills** repository root,
   ...
   - `--omit-garden-families <ids>` — comma-separated column family ids to hide
     from the Agile Garden prelude only (e.g. `utilities`); skills stay in the
     flat grids and outline. Combines with `_CATALOG_GARDEN_PRELUDE_OMIT_FAMILIES`
     in `generate_abd_catalog.py`.
...
3. **Extraction (skills).** For **name** and fallbacks when `README.md` is absent
   ...
   - *Summary (cards + garden rows)* — optional YAML **`catalogue_one_liner`** on
     the entry doc **wins** when set (short card line; keeps long
...
4. **Extraction (agents).**
   ...
   - *Name (cards + detail H1)* — **package directory name** (kebab-case),
...
5. **Hub and grid intros (review with an AI or editor).** Short HTML above the
   grids lives in `skill-builder/skills/abd-skill-catalog/templates/intros/`. Keep copy
   **user-facing** (how to browse, link to outline) — do **not** explain how the
   AI Garden is built (README conventions, generator, scaffolding); that belongs
...
7. **Templates (layout).** HTML shells and CSS live under
   `skill-builder/skills/abd-skill-catalog/templates/` (excluding `intros/`, which are prose
