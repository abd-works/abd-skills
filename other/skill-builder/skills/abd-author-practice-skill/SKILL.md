...
4. **Rewrite the target `SKILL.md` as a thin router.** Replace placeholder voice with plain **Purpose** and **When to use**, then Agent Instructions (**MANDATORY** read `common/skill-workflow.md` + skill-specific § Generate), then Validate. Move concept teaching and examples to **`reference/*.md`** — they do not belong in the SKILL body. Put practice-file links in **`reference/concepts.md`**, not a per-file Read context list in `SKILL.md`. Where you claim something is hub-backed, cite retrieval row and source when that helps a reviewer. Either fill every **`{{PLACEHOLDER}}`**, defer it in writing with a reason, or narrow what the skill promises.
...
- **Thin router shape** — **`SKILL.md`** Agent Instructions: **MANDATORY** read `common/skill-workflow.md` in full; no per-file Read context section (folder reads + `reference/concepts.md` links); § Generate is skill-specific only; validation via `common/skill-workflow.md` § Validate output.
