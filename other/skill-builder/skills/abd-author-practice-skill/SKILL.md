...
- **Thin router shape** — **`SKILL.md`** Agent Instructions opens with **MANDATORY bootstrap**: read **`common/skill-workflow.md` in its entirety**, complete § Read-gates, read every file in § Read context in full; validation re-reads **`rules/`** and emits per-rule verdict; concept teaching lives in **`reference/*.md`**, not **`SKILL.md`** body.
