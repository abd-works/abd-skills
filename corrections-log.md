...
- **Context:** Agent Instructions step 4 and the rule `code-examples-follow-clean-code-and-atdd.md` in `abd-architecture-reference/`; rule `inherit-clean-code-and-atdd-by-reference.md` in `abd-build-architecture-skill/`; the description blocks, sources doc, template, and ide-files of both skills.
...
- **Context:** Document-layout instructions and the `layout-matches-mechanism-count` rule in `abd-architecture-reference/`; reference-copying instructions in `abd-build-architecture-skill/`.
...
- **Context:** `abd-architecture-reference/templates/` originally shipped both `architecture-reference.md` and `mechanism-section.md`.
...
- **Example (wrong):** `Agent Instructions` step 1 in `abd-architecture-reference/SKILL.md` ended with: "The closest worked example of the reference document this skill produces is **[`mern-architecture.md`](../mern-technical-architecture/inputs/mern-architecture.md)** — read it before generating new sections so the shape stays consistent."
...
- **Context:** `Agent Instructions` step 1 and `Build` steps 1–2 in `abd-architecture-reference/SKILL.md`; mirror in `abd-build-architecture-skill/SKILL.md`.
...
- **Context:** `When to use this skill` bullet in `abd-architecture-reference/SKILL.md` describing what an architecture reference fixes.
...
- **Context:** Authoring `When to use this skill` bullets for `abd-architecture-reference/SKILL.md` (and `abd-build-architecture-skill/SKILL.md`).
...
- **Example (wrong):** `abd-architecture-reference/SKILL.md` opened `When to use` with:
