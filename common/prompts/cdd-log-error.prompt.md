...
2. **Log on disk in the same turn:**
   ...
   - **Otherwise:** find or create `skill-errors-log.md` in the **same directory as the artifact that was wrong** (next to the output file, not inside the skill source). Append: DO / DO NOT (forward-looking) + Example (wrong). Leave Example (correct) blank.
