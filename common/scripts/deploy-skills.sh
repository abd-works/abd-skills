...
KNOWN_PACKAGE_FOLDERS=(
    ...
    tests test catalog retired hooks conf
)
...
get_package_roots() {
    ...
    # common/ — shared reference folder deployed to .cursor/skills/common/
    local common_path="$repo_root/common"
    if [[ -d "$common_path" ]]; then
        roots+=("common|$common_path")
    fi
...
deploy_package() {
    ...
    # vscode/ in common/ always merges into .vscode/ regardless of IDE target
    local pkg_name
    pkg_name=$(basename "$package_root")
    if [[ "$pkg_name" == "common" ]]; then
        if [[ "$ide" == "vscode" ]]; then
            remove_and_copy_directory "$package_root" "$github_skills_dst/common"
        else
            remove_and_copy_directory "$package_root" "$skills_dst/common"
            local prompt_src="$package_root/prompt"
            if [[ -d "$prompt_src" ]]; then
                for f in "$prompt_src"/*.prompt.md; do
                    [[ -f "$f" ]] || continue
        ...
        # vscode/ in common/ merges into .vscode/ for any IDE
        local common_vscode_src="$package_root/vscode"
        if [[ -d "$common_vscode_src" ]]; then
            merge_vscode_files "$common_vscode_src" "$deploy_root"
        fi
        # scripts/ in common/ also copies to .cursor/scripts/ (for workspace task references)
        local common_scripts_src="$package_root/scripts"
        if [[ -d "$common_scripts_src" ]]; then
            local cursor_scripts_dst="$deploy_root/.cursor/scripts"
            ensure_directory "$cursor_scripts_dst"
            find "$common_scripts_src" -maxdepth 1 -type f | while IFS= read -r f; do
                cp "$f" "$cursor_scripts_dst/$(basename "$f")"
            done
        fi
        return
    fi
    ...
    # --- Content, reference, lib, conf (cursor only) ---
    if [[ "$ide" != "vscode" ]]; then
        ...
        if [[ -d "$package_root/conf" ]]; then
            local conf_dst="$skills_dst/conf"
            ensure_directory "$conf_dst"
            find "$package_root/conf" -maxdepth 1 -type f | while IFS= read -r file; do
                cp "$file" "$conf_dst/$(basename "$file")"
            done
        fi
    fi
    ...
    # --- Package-level scripts → .cursor/scripts/ ---
    local package_scripts_src="$package_root/scripts"
    if [[ -d "$package_scripts_src" ]]; then
        local cursor_scripts_dst="$deploy_root/.cursor/scripts"
        ensure_directory "$cursor_scripts_dst"
        find "$package_scripts_src" -maxdepth 1 -type f | while IFS= read -r file; do
            cp "$file" "$cursor_scripts_dst/$(basename "$file")"
        done
    fi
    ...
    if [[ -d "$hooks_src" ]]; then
        ...
        # Copy top-level files (e.g. *.json configs) only — scripts live in package scripts/ above
...
if ! $SKIP_CHECKS; then
    ...
    # Encoding scan
    if ! python3 "$REPO_ROOT/common/scripts/scan_encoding.py" --check 2>&1; then
        validation_failed=true
    fi

    # Deploy-path + structure test
    if ! python3 "$REPO_ROOT/common/scripts/test_deploy_paths.py" 2>&1; then
        validation_failed=true
    fi
...
delta_json=$(python3 "$REPO_ROOT/common/scripts/generate_manifest.py" --deployed "$resolved_deploy_root" 2>/dev/null) || true
...
python3 "$REPO_ROOT/common/scripts/generate_manifest.py" --write-receipt "$resolved_deploy_root" --ide "$IDE" 2>/dev/null || true

# Build skill index
skill_index_script="$resolved_deploy_root/.cursor/skills/common/scripts/build_skill_index.py"
