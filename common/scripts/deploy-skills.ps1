...
function Deploy-Package {
    ...
    # vscode/  — merged into deploy root's .vscode/ (tasks.json, settings.json)
    if ((Split-Path -Leaf $PackageRoot) -eq 'common') {
        ...
        $commonVscodeSrc = Join-Path $PackageRoot 'vscode'
        if (Test-Path -LiteralPath $commonVscodeSrc) {
            Merge-VscodeFiles -SourceDir $commonVscodeSrc -DeployRoot $DeployRoot
        }
        # scripts/ in common/ also copies to .cursor/scripts/ (for workspace task references)
        $commonScriptsSrc = Join-Path $PackageRoot 'scripts'
        if (Test-Path -LiteralPath $commonScriptsSrc) {
            $cursorScriptsDst = Join-Path $DeployRoot '.cursor\scripts'
            New-Directory -Path $cursorScriptsDst
            Get-ChildItem -LiteralPath $commonScriptsSrc -File | ForEach-Object {
                Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $cursorScriptsDst $_.Name) -Force
            }
        }
        # instructions/ in common/ deploy to .cursor/rules/*.mdc or .github/instructions/*.instructions.md
        $commonInstructionsSrc = Join-Path $PackageRoot 'instructions'
        if (Test-Path -LiteralPath $commonInstructionsSrc) {
            Get-ChildItem -LiteralPath $commonInstructionsSrc -Filter '*.instructions.md' -File | ForEach-Object {
                if ($Ide -eq 'vscode') {
                    ...
                    Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $rulesDst "$base.mdc") -Force
                }
            }
        }
        return
    ...
    if ($Ide -ne 'vscode') {
        ...
        $confSrc = Join-Path $PackageRoot 'conf'
        if (Test-Path -LiteralPath $confSrc) {
            $confDst = Join-Path $skillsDst 'conf'
            New-Directory -Path $confDst
            Get-ChildItem -LiteralPath $confSrc -File | ForEach-Object {
                Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $confDst $_.Name) -Force
            }
        }
    }
    ...
    $packageScriptsSrc = Join-Path $PackageRoot 'scripts'
    if (Test-Path -LiteralPath $packageScriptsSrc) {
        $cursorScriptsDst = Join-Path $DeployRoot '.cursor\scripts'
        New-Directory -Path $cursorScriptsDst
        Get-ChildItem -LiteralPath $packageScriptsSrc -File | ForEach-Object {
            Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $cursorScriptsDst $_.Name) -Force
        }
    }
...
if (-not $SkipChecks) {
    ...
    # Encoding scan
    try {
        & python3 "$RepoRoot/common/scripts/scan_encoding.py" --check 2>&1 | ForEach-Object { Write-Host $_ }
...
try {
    $deltaJson = & python3 "$RepoRoot/common/scripts/generate_manifest.py" --deployed "$resolvedDeployRoot" 2>$null
    if ($deltaJson) {
...
try {
    & python3 "$RepoRoot/common/scripts/generate_manifest.py" --write-receipt "$resolvedDeployRoot" --ide $ide 2>$null
} catch {
