...
6. **Verify with scanners.** After producing the generated skill, run:
   ...
   ```bash
   python skills/skill-builder/abd-author-practice-skill/scripts/bundle_rules_into_skill_md.py --skill-root skills/engineering/<generated-skill-name>
   python skills/skill-helpers/execute-skill-using-skills-rules/scripts/run_scanners.py \
     --skill-root skills/skill-builder/abd-author-practice-skill \
...
7. **Assembling this skill.** After editing rules in **this** skill, run:
   ...
   ```bash
   python skills/skill-builder/abd-author-practice-skill/scripts/bundle_rules_into_skill_md.py --skill-root skills/engineering/abd-build-architecture-skill
   ```
...
7. **Bundle the generated rules.** From the agilebydesign-skills repo root, run:
   ...
   ```bash
   python skills/skill-builder/abd-author-practice-skill/scripts/bundle_rules_into_skill_md.py --skill-root skills/engineering/<arch-name>-technical-architecture
   ```
...
- Run the bundle script after adding or removing a `scanner:` field so the inlined block in `SKILL.md` stays in sync.
  ...
  **Example (pass):** After adding the scanner field, `python skills/skill-builder/abd-author-practice-skill/scripts/bundle_rules_into_skill_md.py --skill-root skills/engineering/<arch>-technical-architecture` is run, and the inlined rule block now shows the new line.
