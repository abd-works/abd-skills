...
def test_paths_exist() -> None:
    assert execute_using_rules_scripts().is_dir()
    assert (execute_using_rules_scripts() / "run_scanners.py").is_file()
...
def test_build_scanner_env_pythonpath_order_matches_run_scanners() -> None:
    ...
    er = str(execute_using_rules_scripts().resolve())
    assert ops in parts
    assert er in parts
