"""Step 2a Option F — Configurable chunk text coverage with two-pass enrichment.
...
Writes chunk_evidence.json after every batch (always current).

Usage:
    python classify_chunks.py                        # 100% of each chunk
    python classify_chunks.py --chunk-pct 50         # first 50% of each chunk
    python classify_chunks.py --chunk-pct 25 --model gpt-4o
...
def main():
    ...
    print(f"Step 2a Option F — Configurable chunk text coverage")
    ...
    all_chunks = json.loads(Path(args.chunks).read_text(encoding="utf-8"))
    ...
    for batch_num, batch in enumerate(pass1_batches, 1):
        ...
        # Write incrementally
        _write_output(output_path, evidence_by_id, ai_calls, args, len(all_chunks),
    ...
    # Build extended term index: Step 1 concepts + all discovered in Pass 1
    extended_index = {}
    for pair in step1.get("modules_and_epics", []):
        module = pair.get("module", {})
        mname = module.get("name", "")
...
def _write_output(output_path, evidence_by_id, ai_calls, args, chunk_count,
                  chunk_pct, p1_done, p1_total, errors, t_start, status):
    evidence_list = list(evidence_by_id.values())
    total_concepts = sum(e["concept_count"] for e in evidence_list)
    total_rels = sum(e["relationship_count"] for e in evidence_list)
    output_path.write_text(json.dumps({
        "option": "F",
        "description": f"AI reads {chunk_pct:.0f}% of each chunk's text. Code enriches with full text using discovered concepts.",
        "chunk_pct": chunk_pct,
        "ai_calls": ai_calls,
        "ai_model": args.model,
