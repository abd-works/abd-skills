...
When the user asks for an **information architecture**, **site map**, **screen flow**, **screen inventory**, **navigation**, or **content model** for an increment, epic, sprint, or other scope, use **`skills/user-experience-design/abd-information-architecture/SKILL.md`**. Read that skill and its bundled rules before drawing.
...
3. **Author `docs/ux/initial-ia.md` first**, then produce `docs/ux/initial-ia.drawio` using one of two modes:
   - **Internal:** agent generates the draw.io XML directly (white boxes, black outlines) and writes `initial-ia.drawio`.
   - **External:** fill `templates/initial-ia-prompt.md` from `initial-ia.md`, output the filled prompt in chat — user pastes into any AI tool, saves the returned XML as `initial-ia.drawio`.
   Sync any diagram changes back into `initial-ia.md`.
4. **Log mistakes inside this skill — immediately, same turn.** When output is wrong, follow the workspace `correct-output` rule and write the entry to `skills/user-experience-design/abd-information-architecture/corrections-log.md` in the same turn you fix the output.
...
- Editing `initial-ia.drawio` for structural changes without updating `initial-ia.md` — always keep the two in sync.
