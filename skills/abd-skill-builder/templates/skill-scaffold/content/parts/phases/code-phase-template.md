...
- **Live checklist path:** `<active_skill_workspace>/{{skill_name}}/progress/{{code_phase_slug}}-checklist.md`. On first run of `python scripts/base/generate.py --phase {{code_phase_slug}}`, that file is created from **## Action Checklist** below if it does not exist yet.
- **Resume point:** the first unchecked box in your **workspace** checklist is where you continue after any interruption.
- **Do not modify this file during a run** to record progress. If the script behaviour changes, update the phase doc and commit.
...
<!-- Canonical action list. Copied to `<active_skill_workspace>/{{skill_name}}/progress/{{code_phase_slug}}-checklist.md` when missing (see generate.py). Tick there, not here. -->
