...
| `skill-config.json` | **Phase manifest** — `phase_files`, `PHASE_LIBRARY_SLICES`, `phase_rules`, `every_phase_rules`, `phase_critical_quality_notes`, **`operator.build_pipeline`** (post-merge steps; rule-bound scanners listed in `rules/scanners.json`) |
