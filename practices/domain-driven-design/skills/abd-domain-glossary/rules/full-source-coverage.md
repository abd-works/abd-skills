...
Every file in the workspace's source directories must be referenced by at least one `Source:` line across all module files (`domain/modules/*.md`) — allocated to a module, placed in `unallocated.md`, or explicitly placed in `rejected.md`. No source file may be silently dropped.
