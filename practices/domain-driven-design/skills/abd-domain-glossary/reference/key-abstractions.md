...
Shared reference for the domain-discovery skills (`abd-domain-glossary`, `abd-domain-language`). This file owns the definition of a Key Abstraction and the candidate-KA decision tests. Each skill owns its own notation, format, and any fidelity-specific elaboration.
...
## Two tests for every candidate KA
...
**2. Fit test.** Does this concept fundamentally connect to the core purpose of the scope being modeled, or does it only touch it tangentially? If it belongs more squarely in an adjacent area, place it at the boundary or move it out.

## Three outcomes for each candidate KA
