...
- **Domain Specification** (`abd-domain-implementation`) — full typed Class Model with typed properties (`+ name: Type`), typed method signatures, ownership semantics. Output file: `class-model.md` or `<name>-class-model.md`. The diagram is a full UML class diagram with typed property and operation compartments.
