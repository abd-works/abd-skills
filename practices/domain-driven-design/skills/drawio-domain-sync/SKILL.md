...
description: >-
  Render Ubiquitous Language, CRC, or object model artifacts to Draw.io class
...
Read these files:
- **`reference/concepts.md`** — source types (ULL, CRC, object model), page-per-KA convention, AI-driven layout, sync-back rules, incremental vs full rendering, build script persistence, and CLI reference pointer.
...
1. **Identify the source file** — locate `<name>-ubiquitous-language.md`, `<name>-crc.md`, or `<name>-object-model.md`. Prefer highest fidelity available.
...
- **Source identified** — a ULL, CRC, or object model file was located before any diagram work began; the agent read the full file.
...
- **ULL bullets become rows** — when source is ULL, every behavior bullet renders as a row, every italicized term resolves to a collaborator, cross-concept references are folded into one association edge, subtype headings produce inheritance edges, boundary stubs render as imported cards.
