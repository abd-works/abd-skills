"""Shared workspace discovery for abd-domain-implementation scanners."""
...
def build_object_model_context(
    ...
    """Collect class-model markdown files under common engagement layouts."""
    del story_graph  # reserved for future graph-aware scans
    files: List[Path] = []

    direct = workspace / "docs" / "domain" / "class-model.md"
    if direct.is_file():
        text = direct.read_text(encoding="utf-8")
        if "state: class-model" in text[:300]:
            files.append(direct)
