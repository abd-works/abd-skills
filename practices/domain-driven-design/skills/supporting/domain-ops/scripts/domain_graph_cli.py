...
Add ``…/domain-ops/scripts`` to PYTHONPATH, then:

  python domain_graph_cli.py read --file path/to/domain-model.json
  python domain_graph_cli.py names --file path/to/domain-model.json
  python domain_graph_cli.py search --file ... --substring "Check"
  python domain_graph_cli.py filter --file ... --modules "Catalog","Check Resolution"
  python domain_graph_cli.py filter --file ... --classes "Check","Trait"
  python domain_graph_cli.py sha --file path/to/domain-model.json
  python domain_graph_cli.py write --file out.json < input.json
"""
...
def _collect_class_names(data: Dict[str, Any]) -> List[str]:
    from domain_map import DomainMap

    return DomainMap(data).class_names()
...
def cmd_names(args: argparse.Namespace) -> int:
    data = _load_and_validate(Path(args.file))
    for name in _collect_class_names(data):
        print(name)
    return 0
...
def cmd_search(args: argparse.Namespace) -> int:
    ...
    hits = [n for n in _collect_class_names(data) if sub in n.lower()]
    for name in hits:
        print(name)
...
def cmd_filter(args: argparse.Namespace) -> int:
    ...
    data = _load_and_validate(Path(args.file))
    if args.modules:
        names: Set[str] = {s.strip() for s in args.modules.split(",") if s.strip()}
        out = filter_domain_model_to_module_names(data, names)
    elif args.classes:
        names = {s.strip() for s in args.classes.split(",") if s.strip()}
        out = filter_domain_model_to_class_names(data, names)
    else:
        print("[error] filter requires --modules or --classes", file=sys.stderr)
        return 2
    if args.pretty:
...
def main(argv: List[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="domain_graph_cli", description="Domain model file operations")
    ...
    n = sub.add_parser("names", help="List class names (flat)")
    n.add_argument("--file", required=True)
    n.set_defaults(func=cmd_names)

    s = sub.add_parser("search", help="List class names containing substring")
    s.add_argument("--file", required=True)
    s.add_argument("--substring", required=True)
    s.set_defaults(func=cmd_search)

    f = sub.add_parser("filter", help="Emit graph subset by module or class names")
    f.add_argument("--file", required=True)
    f.add_argument("--modules", default=None, help="Comma-separated module names")
    f.add_argument("--classes", default=None, help="Comma-separated class names")
