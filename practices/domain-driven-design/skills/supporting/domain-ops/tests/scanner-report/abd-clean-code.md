# Scanner Report — abd-clean-code

**Workspace:** practices/domain-driven-design/skills/supporting/domain-ops/scripts
**Date:** 2026-06-29 07:21:00

---

## Scanner Execution Status

### 🟩 Overall Status: ALL CLEAN

| Status | Count | Description |
|--------|-------|-------------|
| 🟩 Executed Successfully | 17 | Scanners ran without errors |
| 🟩 Clean Rules | 17 | No violations found |

**Total Rules:** 17
- **Rules with Scanners:** 17
  - 🟩 **Executed Successfully:** 17

---

### Scanner Results

| Status | Rule | Violations |
|--------|------|------------|
| 🟩 CLEAN | Abstraction Levels Scanner | 0 |
| 🟩 CLEAN | Clear Parameters Scanner | 0 |
| 🟩 CLEAN | Consistent Naming Scanner | 0 |
| 🟩 CLEAN | Domain Language Code Scanner | 0 |
| 🟩 CLEAN | Duplication Scanner | 0 |
| 🟩 CLEAN | Exception Handling Scanner | 0 |
| 🟩 CLEAN | Explicit Dependencies Scanner | 0 |
| 🟩 CLEAN | Function Single Responsibility Scanner | 0 |
| 🟩 CLEAN | Function Size Scanner | 0 |
| 🟩 CLEAN | Intention Revealing Names Scanner | 0 |
| 🟩 CLEAN | Meaningful Context Scanner | 0 |
| 🟩 CLEAN | Property Encapsulation Code Scanner | 0 |
| 🟩 CLEAN | Separate Concerns Scanner | 0 |
| 🟩 CLEAN | Simplify Control Flow Scanner | 0 |
| 🟩 CLEAN | Single Responsibility Scanner | 0 |
| 🟩 CLEAN | Swallowed Exceptions Scanner | 0 |
| 🟩 CLEAN | Useless Comments Scanner | 0 |

---

## Violations

_No violations found._
