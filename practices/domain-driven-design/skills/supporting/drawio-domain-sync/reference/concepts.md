...
- **Domain Specification** (`abd-domain-specification`) — full typed domain specification with typed properties (`+ name: Type`), typed method signatures, ownership semantics. Output file: `domain-specification.md` or `<name>-domain-specification.md`. The diagram is a full UML class diagram with typed property and operation compartments.
