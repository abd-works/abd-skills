"""Shared workspace discovery for abd-domain-model scanners."""
...
def build_domain_model_context(
    workspace: Path,
    story_graph: Path | None = None,
) -> ScanFilesContext:
    """Collect domain-model markdown files under common engagement layouts."""
    del story_graph
    files: List[Path] = []

    direct = workspace / "docs" / "domain" / "domain-model.md"
    if direct.is_file():
