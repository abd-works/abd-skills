...
# Class Model section
_OM_SECTION_RE = re.compile(r"^### Class Model\s*$", re.MULTILINE)
...
def _find_object_model_sections(lines: List[str]) -> List[tuple[int, int]]:
    """Return (start_line, end_line) for each ### Class Model section."""
    sections = []
    start = None
    for i, line in enumerate(lines):
        if re.match(r"^### Class Model\s*$", line):
            start = i
        elif start is not None and re.match(r"^#{2,3}\s", line) and not re.match(r"^### Class Model", line):
