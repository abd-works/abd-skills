...
Variable names inside an `Interaction:` block must use domain language drawn from the Domain Language, Domain Language, or domain model — not generic technical placeholders. If a name is ambiguous, trace up the chain: domain model responsibility ? Domain Language ? Domain Language ? source references.
...
Trace up the chain:
1. Check the domain model responsibility name and its collaborator names
2. Check the Domain Language description of the behavior
3. Check the Domain Language definition
