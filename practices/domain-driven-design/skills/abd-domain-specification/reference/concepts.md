...
- **Variable names must use domain language** — `roll`, `margin`, `throwingDistanceMeasure`, not `r`, `temp`, `measureA`. Trace the right name up the chain: domain model ? Domain Language ? source references.
...
Code is the source of truth at specification fidelity. One file per Key Abstraction, named after the KA (`customer.ts`, `cart.ts`, …). Same file evolves from **model fidelity** (real types only) to **specification fidelity** (adds `@stereotype`, `@composition`/`@aggregation`/`@association`, invariant methods, interaction methods) — a *new file is not created* when fidelity increases (D26).

### Emission target: abstract class

Emit **abstract classes** in the target language — not interfaces — so the class can carry constructors, initialisation policy, stereotype markers, and empty invariant/interaction methods.
...
Templates: [`../templates/domain-specification.ts`](../templates/domain-specification.ts), [`../templates/domain-specification.py`](../templates/domain-specification.py), [`../templates/domain-specification.java`](../templates/domain-specification.java).

### What the type system carries

Real types express **types, properties, operation signatures, return types, subtypes**. Do not restate any of that in doc comments — the compiler already knows.

### Structural markers — locked doc-comment tag dictionary

A small set of structural markers ride in JSDoc / docstrings / Javadoc. These tags are **parser-readable but not runtime-enforced** — they exist so `code → canonical JSON` conversion has something concrete to walk.

| Tag                                              | Placement    | Values                                                                        |
| ------------------------------------------------ | ------------ | ----------------------------------------------------------------------------- |
...
| `@composition` / `@aggregation` / `@association` | on property  | (bare; kind is the tag itself)                                                |
| `@invariant`                                     | on empty method | (bare; the method name IS the rule)                                        |
| `@interaction`                                   | on empty method | (bare; the method name IS the summary)                                     |

No free-text arguments. Anything that wants prose goes in the sibling `domain-context.md` (D14).

### Invariants and interactions as empty methods (D24)

An invariant is written as an **empty abstract method** whose name captures the rule:
...
The method never gets a body. It exists so the canonical graph can list the invariant, so scanners can enforce naming rules, and so IDE outline views make invariants first-class alongside operations.

An interaction follows the same pattern:
...
**No `@invariant "free text rule"` and no step-by-step interaction bodies.** Step-by-step narrative belongs in the concrete implementation of the operation (regular code comments inside the method body) or in `domain-context.md` if it must be preserved at the spec level. Legacy markdown specs with `Interaction:` blocks migrate by moving those blocks into `domain-context.md` under a `TODO: migrate interaction narrative` heading; the code carries only the empty `@interaction` method.

### Phase grouping — region banners (D21)

Group operations by phase (`Onboarding`, `Self-care`, `Billing`, …) using the target-language region convention:

- TypeScript / JavaScript → `//region <phase>` … `//endregion`   (VS Code + WebStorm)
- Python                  → `# region <phase>` … `# endregion`   (VS Code)
- Java                    → `//region <phase>` … `//endregion`   (IntelliJ)

Phase grouping is a display concern only — it never affects semantics and never affects the canonical JSON graph beyond a `phase` field on each operation.

### File / folder granularity (D17)

- Default: **one folder per module, one file per KA** (`customer/customer.ts`, `cart/cart.ts`).
- Domain-oriented codebases with both back-end and front-end files quickly need **one folder per KA** so the multiple concrete implementations live together.
- The bootstrapping `generate` command asks the user which layout to emit; both are supported. After bootstrap, the choice is fixed by what exists on disk.

### Boundary code

Boundary KAs are just regular files in another folder, imported normally:

```typescript
import type { Billing } from '../billing/billing'
```

The nature of the dependency (why this boundary was drawn, what flows across it) belongs in `domain-context.md` at the module folder root, not in code comments.
