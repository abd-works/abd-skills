...
**Pull:** When a ticket is `stage: engineering` and active, agents pull skills from `kanban.json` for this stage ? same [
...
Deliver working software for the slice: runnable UI from `abd-ux-specification`, typed domain implementation, failing then passing tests, and clean production implementation ? honoring specification artifacts (scenarios, interface spec, domain model, architecture reference).
...
| 2 ? `abd-domain-specification` (domain implementation) | **Business Expert** | Reviewer |
...
| 2 | **Domain-driven design** | `abd-domain-specification` | Business Expert | **Domain implementation** ? typed code surface from domain model; aligned with scenarios and architecture reference |
|| 2b | **Domain-driven design** | `abd-domain-code` | Business Expert | **Optional** — domain tests + production code TDD when doing domain-only work; replaces separate ATDD + clean-code for domain layer |
...
- [Specification](specification.md) exit gate passed ? including `interface-design.md` from `abd-ux-specification` **spec pass**, `domain-model.md` from `abd-domain-model`, and architecture reference when in scope.
...
1. Scanners green for **each assigned skill** in order (`run_scanners.py` for `abd-ux-specification`, `abd-domain-specification`, `abd-acceptance-test-driven-development`, `abd-clean-code` as applicable).
2. Domain implementation aligns with domain model when `abd-domain-specification` ran.
