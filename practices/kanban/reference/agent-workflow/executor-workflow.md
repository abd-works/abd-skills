...
| `abd-story-acceptance-criteria` | `acceptance_criteria[]` on every in-scope story — must be non-empty |
| `abd-story-specification` | `scenarios[]` / `scenario_outlines[]` on matched stories |
