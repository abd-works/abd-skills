...
Produces `docs/ux/information-architecture/information-architecture.drawio` � two pages: Page 1 (Detailed IA, full screen layouts) and Page 2 (Site Map, one box per screen with labeled arrows). Must exist on disk before the cell is marked done.

Resolve the CLI path once:
```powershell
# From the abd-skills repo:
$DUXCLI = "node skills/user-experience-design/abd-information-architecture/scripts/drawio-ux.mjs"
# From a deployed project junction:
$DUXCLI = "node .cursor/skills/abd-information-architecture/scripts/drawio-ux.mjs"
```

Minimal invocation sequence (full commands in `## Build`):
```powershell
node $DUXCLI open docs/ux/information-architecture/information-architecture.drawio
node $DUXCLI clear
# ... add-screen, add-list/add-form/add-chrome, connect, add-callout calls per screen
node $DUXCLI save
```

`save` writes both pages. Overwrite on every run � git carries the history.

---

## Build

**Goal:** Read the story map and ubiquitous language for a scope, identify the screens, transitions, navigational components, content types, per-screen content layouts, labels, and key actions, and save the result as `information-architecture.md` (structured spec) and `information-architecture.drawio` (diagram) in the engagement's `docs/ux/` folder.
...
10. **Author or update `docs/ux/information-architecture/information-architecture.md` first.** Copy `templates/information-architecture.md` into the engagement's `docs/ux/` folder (or open the existing one) and fill in: scope, source paths (story map, UL), description, screens (regions, content types, actions, in-scope story names with links, grouped system story names with links), transitions, navigational components, and content-type details. This markdown is the structured spec the canvas is drawn from; the canvas is never authored without it.
...
10a. **Run the completeness test before touching the canvas.** Walk the checklist in `rules/tab-states-and-domain-traceability.md`:
  ...
  Only proceed to the canvas when all items pass. Fix gaps in `information-architecture.md` first.

11. **Choose output mode.** After `information-architecture.md` passes the completeness check, produce `information-architecture.drawio` using one of two modes � ask the user if not already specified:
    ...
    **Mode A � Internal (agent uses the `drawio-ux` CLI):**
    ...
    # ... one add-screen per screen in information-architecture.md
    ...
    Overwrite `docs/ux/information-architecture/information-architecture.drawio` on every run � git carries the history.

    **Mode B � External (output a filled prompt for use anywhere):**
    Fill the slots in `templates/information-architecture-prompt.md` from `docs/ux/information-architecture/information-architecture.md` and **output the filled prompt to the user in chat**. The user pastes it into any AI chat (ChatGPT, Claude.ai, Copilot, Cursor, etc.) and asks the AI to return draw.io XML. The prompt instructs the AI to follow the same style contract as Mode A (white boxes, black outlines) and return only raw mxGraph XML. Save the returned XML to `docs/ux/information-architecture/information-architecture.drawio`.

12. **Sync diagram changes back into `information-architecture.md`.** If the diagram was edited directly in draw.io or via the drawio-ai plugin and structural changes were made (screens renamed, regions added/removed, transitions changed), update `docs/ux/information-architecture/information-architecture.md` to match. Append a row to the change log with date, direction (`drawio ? md`), and a one-line summary.
...
- **Outputs:** `docs/ux/information-architecture/information-architecture.md` (structured spec) and `docs/ux/information-architecture/information-architecture.drawio` (diagram) � two artifacts in the engagement's deliverables folder, kept in sync.
...
- **Cross-artifact parity:** `information-architecture.md` and `information-architecture.drawio` describe the same IA � the same screens, transitions, navigational components, content types, and actions.
...
- `docs/ux/information-architecture/information-architecture.md` and `docs/ux/information-architecture/information-architecture.drawio` both exist and are in sync.
