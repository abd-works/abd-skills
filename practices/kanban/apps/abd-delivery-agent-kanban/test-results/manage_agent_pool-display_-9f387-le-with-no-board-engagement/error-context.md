...
- Name: manage_agent_pool\display_agent_pool\show_agent_liveness_in_stage_pool_e2e.spec.ts >> Indicate Agent Liveness from Heartbeat >> Scenario 2: Agent inactive — heartbeat stale with no board engagement
- Location: tests\e2e\manage_agent_pool\display_agent_pool\show_agent_liveness_in_stage_pool_e2e.spec.ts:29:7
...
Locator: locator('.kb-pool-group[data-role="business-expert"] [title]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.kb-pool-group[data-role="business-expert"] [title]').first()
