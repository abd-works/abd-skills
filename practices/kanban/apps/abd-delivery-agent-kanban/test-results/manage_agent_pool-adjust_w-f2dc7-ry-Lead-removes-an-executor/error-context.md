...
- Name: manage_agent_pool\adjust_wip_policy\scale_agent_pool_up_or_down_e2e.spec.ts >> Scale Agent Pool Up or Down >> Scenario 2: Delivery Lead removes an executor
- Location: tests\e2e\manage_agent_pool\adjust_wip_policy\scale_agent_pool_up_or_down_e2e.spec.ts:32:7
...
Call log:
  - waiting for locator('.kb-pool-group[data-role="business-expert"]').locator('button[title^="Remove Business Expert"]').first()
