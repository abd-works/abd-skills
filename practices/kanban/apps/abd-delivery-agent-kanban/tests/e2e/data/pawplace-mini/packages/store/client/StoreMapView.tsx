import React, { useCallback, useEffect, useState } from 'react';
import { tokens } from '../../shared/layout-tokens';
import type { CustomerLocation, StoreRecord, StoreWithDistance } from '../shared/types';
import { useConfirmSelectedStore, useSelectedStore } from './SelectedStoreContext';
import { fetchStores, fetchStoresWithDistance, resolveBrowserLocation } from './store.api';

export interface StoreMapViewProps {
  initialLocation?: CustomerLocation | null;
}

export function StoreMapView({ initialLocation = null }: StoreMapViewProps) {
  const { selectedStore, setSelectedStore } = useSelectedStore();
  const confirmSelection = useConfirmSelectedStore();
  const [stores, setStores] = useState<StoreWithDistance[]>([]);
  const [customerLocation, setCustomerLocation] = useState<CustomerLocation | null>(
    initialLocation,
  );
  const [locationError, setLocationError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  const loadStores = useCallback(async (location: CustomerLocation | null) => {
    setLoading(true);
    const data = location
      ? await fetchStoresWithDistance(location)
      : await fetchStores();
    setStores(data);
    setLoading(false);
  }, []);

  useEffect(() => {
    void loadStores(customerLocation);
  }, [customerLocation, loadStores]);

  const handleUseMyLocation = async () => {
    setLocationError(null);
    try {
      const location = await resolveBrowserLocation();
      setCustomerLocation(location);
    } catch {
      setLocationError('Could not access location. Try again or browse stores without distance.');
    }
  };

  const handleSelectStore = (store: StoreRecord) => {
    setSelectedStore(store);
  };

  if (loading) {
    return <p>Loading stores…</p>;
  }

  return (
    <section aria-label="Find Store — Map" data-testid="store-map-view">
      <div
        role="listbox"
        aria-label="store map"
        data-testid="store-map-listbox"
        style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing.sm }}
      >
        {stores.map((store) => {
          const isSelected = selectedStore?.id === store.id;
          return (
            <div
              key={store.id}
              role="option"
              aria-selected={isSelected}
              data-testid="store-map-entry"
              tabIndex={0}
              onClick={() => handleSelectStore(store)}
              onKeyDown={(event) => {
                if (event.key === 'Enter' || event.key === ' ') {
                  event.preventDefault();
                  handleSelectStore(store);
                }
              }}
              style={{
                ...tokens.body,
                padding: tokens.spacing.md,
                background: isSelected ? tokens.surfaceMuted : tokens.surface,
                border: `1px solid ${tokens.surfaceMuted}`,
                borderRadius: 4,
                cursor: 'pointer',
              }}
            >
              <span aria-hidden="true">📍 </span>
              <strong>{store.retailLocationIdentity}</strong>
              <span> — {store.geographicPlacement}</span>
              {store.distanceToStoreKm !== undefined && (
                <span data-testid="distance-to-store">
                  {' '}
                  · distance to store {store.distanceToStoreKm.toFixed(1)} km
                </span>
              )}
            </div>
          );
        })}
      </div>

      {selectedStore && (
        <aside
          data-testid="selected-store-detail"
          style={{ marginTop: tokens.spacing.md, ...tokens.body }}
          aria-live="polite"
        >
          <p>
            <strong>{selectedStore.retailLocationIdentity}</strong>
            <br />
            {selectedStore.geographicPlacement}
          </p>
        </aside>
      )}

      <div
        role="toolbar"
        aria-label="store map actions"
        style={{ marginTop: tokens.spacing.lg, display: 'flex', gap: tokens.spacing.md }}
      >
        <button type="button" onClick={() => void handleUseMyLocation()}>
          use my location
        </button>
        <button
          type="button"
          disabled={!selectedStore}
          onClick={confirmSelection}
          style={{
            background: tokens.accent,
            color: tokens.surface,
            border: 'none',
            padding: `${tokens.spacing.sm}px ${tokens.spacing.md}px`,
            cursor: selectedStore ? 'pointer' : 'not-allowed',
          }}
        >
          select store
        </button>
      </div>

      {locationError && (
        <p role="alert" style={{ color: tokens.accent, marginTop: tokens.spacing.sm }}>
          {locationError}
        </p>
      )}
    </section>
  );
}
