...
_SCRIPT_DIR = Path(__file__).resolve().parent.parent.parent  # → tests/domain/python/
...
from operate_board_in_manual_mode.operate_board_in_manual_mode_helper import (
    given_action_intent,
...
# ============================================================================
# STORY: Execute Assigned Skill on Ticket — fixture mode
# ============================================================================

_E2E_SEED_STUBS = Path(__file__).resolve().parents[4] / "e2e" / "_seed" / "pawplace-stubs"


@pytest.fixture
def stub_workspace(tmp_path: Path) -> Path:
    import shutil
    shutil.copytree(_E2E_SEED_STUBS, tmp_path, dirs_exist_ok=True)
    return tmp_path


class TestExecuteAssignedSkillOnTicketFixtureMode:
    """Execute Assigned Skill on Ticket — skill fixture harness (fixture mode)."""

    def test_is_fixture_mode(self, stub_workspace: Path):
        from delivery_model import is_fixture_mode
        assert is_fixture_mode(stub_workspace) is True

    def test_pick_fixture_entry_scope_all(self, stub_workspace: Path):
        import json
        from agent import TeamMemberAgent

        fixtures = json.loads((stub_workspace / "skill-fixtures.json").read_text(encoding="utf-8"))
        entry = TeamMemberAgent._pick_fixture_entry(fixtures, "abd-story-mapping", "all")
        assert any(c["target"].endswith("shaping/story-map.md") for c in entry["copies"])

    def test_apply_module_partition(self, stub_workspace: Path):
        ...
        result = TeamMemberAgent(stub_workspace, "business-expert", instance=1).apply_skill_fixture(
            "project-all", "abd-domain-partition"
        )
