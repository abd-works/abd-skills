...
| `abd-domain-code` | Domain-driven design | **Engineer** (Business Expert reviews) |
...
The kanban lead bootstraps **eight agents once** per engagement. Each agent pulls skills from the **Kanban board** (`board.json`) — see [../../content/kanban.md](../kanban.md) and [../../agents/_shared/work-queue.md](../../agents/_shared/work-queue.md).
...
| `abd-story-specification` | Specification | story-driven delivery | After domain model; outline tables use domain model concepts |
...
| `abd-domain-walk` | Specification | domain-driven design | **After** spec-by-example |
| **`abd-domain-code` review** | Engineering | domain-driven design | Checkpoint only — Engineer produces |
...
| `abd-domain-code` | Engineering | domain-driven design | Typed domain surface; BE reviews |
...
**Engineering pass order:** `abd-interface-design` impl (UX) → domain specification → ATDD → clean code (+ stack). Engineer runs steps 2–4.
...
| **Engineering** | interface design (impl, UX) → domain specification (Engineer) → ATDD (Engineer) → clean code + stack (Engineer) |
