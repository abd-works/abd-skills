...
"Run all five bootcamp stages start to finish in one run" is the **degenerate case** for trivial engagements with no classified risk. Real plans decompose the work into targeted runs that flush out unknowns early, per `abd-kanban-planning` Â§2a“2c.
...
- Use strategies from `reference/strategies/` as starting points ” they decompose the run structure for you.
...
- Hide the sweep behind renamed stages ” use canonical stage names from the stage files in `reference/stages/`.
