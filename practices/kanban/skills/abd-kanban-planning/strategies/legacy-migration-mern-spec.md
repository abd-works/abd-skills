...
| Shaping | **project** | **module-partition** (detailed — use `code-research/` + `context/BESS28 Source/`), **bounded-context-map**, **story-mapping** (outline from legacy flows), **architecture-outline** (MERN target — read `mern-technical-architecture`) |
...
| Exploration | **increment** | **ubiquitous-language**, **acceptance-criteria** (legacy behavior is spec), **ux-mockup** (optional), **architecture-reference** (assign/create MERN mechanism reference from blueprint — gate at execution) |
...
| Engineering | **sprint** | **ATDD RED only (PO)** (tests for new system; legacy research + spec as oracle — **no old-system execution**). No object-model, clean-code, or GREEN — engagement stops after RED tests. |
...
- **`mern-technical-architecture`** — domain-first `packages/<domain>/{shared,server,client,tests}`.
...
**Partitioning enabled** (`abd-module-partition` in shaping). Four ticket tiers: **project → partition → increment → sprint**.
