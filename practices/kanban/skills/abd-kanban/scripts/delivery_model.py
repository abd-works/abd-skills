...
Machine files under `<workspace>/docs/planning/kanban/`:
  - `kanban.json` — kanban board stage configuration
  - `board.json` — board state (backlog, active, done, archived, team)
...
@dataclass
class StageDef:
    """Stage configuration from kanban.json.  Carries the stage work required."""
    name: str
    scope: str
...
class Ticket:
    ...
    def advance_to_next_stage(self, next_stage_def: StageDef) -> None:
    ...
    def next_eligible_skill(self, stage_def: StageDef, role: str) -> str | None:
...
class KanbanBoard:
    ...
    def next_stage(self, current_stage: str) -> StageDef | None:
    ...
    ) -> tuple[Ticket, str, StageDef] | None:
