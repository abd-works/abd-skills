...
Produces two diagrams alongside `architecture-specification.md`. Both must exist before the cell is marked done.

**`architecture-flow.drawio`** — start from `templates/architecture-flow.drawio`; replace every placeholder label with the actual layer or file name for this architecture. Plain boxes and lines only. No script — fill the template manually and save as `architecture-flow.drawio` next to `architecture-specification.md`.

**`architecture-specification-participants.drawio`** — UML class diagram of the 14 domain module participants. Run from the spec directory:
