...
- Copy or summarize the flow diagram and tech table from the architecture's agreed source of truth into **Architecture Flow**, keeping tech names and domain placeholders aligned with the source.

  **Example (pass):** The MERN named spec defines Web Client, App Server, Persistence as mechanisms and an Instantiating the Domain section — a new MERN project reference assigns those sections verbatim rather than inventing Error Handling as a fourth mechanism.
...
**Source:** Practice-skill authoring convention (abd-architecture-specification); preserves the vocabulary contract between the reference and the team's agreed architecture context, wherever that context lives.
