...
1. **Ask the user** whether to sync diagram changes back to `story-graph.json` (using `drawio_story_sync_cli.py sync`). Same prompt/auto/no convention.
2. After syncing, companion diagrams (exploration, increments) are automatically refreshed by the `sync` command.
