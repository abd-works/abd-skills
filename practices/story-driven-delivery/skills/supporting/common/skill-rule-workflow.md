# Skill Rule Workflow — Story-Driven Delivery (Supporting Skills)

Shared workflow for story-driven-delivery **supporting** practice skills. Skills reference this file for read-gates, output file resolution, and per-rule verdict format.

See the canonical version at `../common/skill-rule-workflow.md` (one level up, under `skills/common/`). This file re-exposes the same content at `skills/supporting/common/` so that supporting skill SKILL.md files can use the same `../common/` relative reference without a path change.
...
4. **Canonical scaffold path.** If none of the above applies, use the path for this skill from [`common/folder-conventions.md`](../../../../../common/folder-conventions.md). The scaffold tree there defines the default `docs/` subtree for every skill family.
