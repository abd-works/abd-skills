...
2. **story-graph-ops** `scripts/` — auto-prepended by `drawio_story_sync._bootstrap` and `story_io_synchronizer` when the sibling skill exists at `skills/supporting/story-graph-ops/scripts`. You can add it explicitly to `PYTHONPATH` if your layout differs.
