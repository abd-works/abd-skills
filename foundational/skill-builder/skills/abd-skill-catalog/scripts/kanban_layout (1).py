...
BOOTCAMP_CATALOG_PREFIX = "../../../../agilebydesign-skills/catalog/"
...
def _kanban_spotlight_source_path(repo_root: Path) -> Path:
    """Where spotlight/header HTML is read for catalog kanban regen."""
    catalog = repo_root / "catalog" / "kanban-layout" / "index.html"
    part3 = _bootcamp_part3_path(repo_root)
    if part3.is_file():
        text = _read_bootcamp_part3(part3)
        if "kb-slide--catalog-kanban" in text or "CATALOG_KANBAN_EMBED" in text:
            if catalog.is_file():
                return catalog
    return part3
...
def extract_bootcamp_slide_headers(repo_root: Path) -> dict[int, str]:
    ...
    for slide_num in range(1, 7):
        ...
        if header:
            out[slide_num] = _catalogize_spotlight_html(header)
    return out
...
def build_kanban_slides_deck_html(
    ...
    """Vertical kb-slide stack inside #m-agentic-delivery (bootcamp part3 / Reveal)."""
    headers = extract_bootcamp_slide_headers(repo_root)
    spotlight_bodies = build_catalog_stage_spotlight_bodies(repo_root)
    legend = build_kanban_legend_html(relative_href_prefix="../")
