...
```powershell
python guidance/workspace/scripts/set_workspace.py "C:\absolute\path\to\project"
```

If **`skill-config.json`** is missing, **`set_workspace.py`** creates it when the repo root can be detected (**`skills/`** + **`scripts/deploy-skills.ps1`**).
...
- Rule: **`guidance/workspace/workspace.mdc`**
- Details: **`guidance/workspace/README.md`**
