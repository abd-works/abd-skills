...
1. **SKILL discovers, AGENTS orchestrates:** **`SKILL.md`** stays short (when to use, commands). Full workflow lives in **`AGENTS.md`** merged from parts. Orchestrator capabilities are **agents** (**`AGENTS.md`** + config), not a second orchestrator **`SKILL.md`**. See **[agent-skill-model.md](library/base/agent-skill-model.md)**.
2. **Author prompts from parts:** Build injected text from **`content/parts/library/`**, **`content/parts/phases/`**, **`rules/`**, and **`content/parts/process.md`** — not ad-hoc chat. Use **`skill-scaffold/`** as the folder blueprint.
3. **Process table is the map:** Every phase slug in **`skill-config.json` → `phase_files`** has a row below (seven columns per **[process-phases.md](library/process-phases.md)**) and a file at **`content/parts/phases/<slug>.md`**. Phase **0** is always **Workspace and config**; its row matches the scaffold contract.
4. **Quality and validation:** **Rules** → **scanners** → fix → **`python scripts/base/build.py`**. If there are no rules, use a **corrections log** under **`active_skill_workspace`** (see **[critical-quality-steps.md](library/critical-quality-steps.md)**). Fix **sources** under **`content/parts/`**, **`rules/`**, **`scripts/`** — not hand-edited **`AGENTS.md`**. See **[rules-and-scanners.md](library/rules-and-scanners.md)**.
5. **Confirm before you lock:** **Analyze** and propose components (1–2 sentences each); user corrects. **Stage 2** then runs phase **#2** through **#6** (scaffold through scripts); **confirm at natural breakpoints** inside that stage before **Stage 3** validation.
