# Create directory junctions so Cursor discovers extract-domain-logic (~/.cursor/skills/ and repo .cursor/skills/).
[CmdletBinding()]
param(
    [switch]$UserOnly,
    [switch]$ProjectOnly
...
if (-not $ProjectOnly) {
    ...
    New-JunctionSafe -Link "$userSkills\extract-domain-logic" -Target $skillRoot
}

if (-not $UserOnly) {
    Write-Host "Project-level junction (.cursor/skills/):" -ForegroundColor Cyan
    New-JunctionSafe -Link "$projectSkills\extract-domain-logic" -Target $skillRoot
}
