<#
.SYNOPSIS
  Create directory junctions so Cursor discovers the key-abstraction-identification skill globally
  (~/.cursor/skills/) and under agilebydesign-skills/.cursor/skills/.

.DESCRIPTION
  Canonical source: agents/abd-ooad/skills/key-abstraction-identification/. Junctions point here — no copies.
...
# key-abstraction-identification -> skills -> abd-ooad -> agents -> repo root
...
if (-not $ProjectOnly) {
    ...
    New-JunctionSafe -Link "$userSkills\key-abstraction-identification" -Target $skillRoot
}

if (-not $UserOnly) {
    Write-Host "Project-level junction (.cursor/skills/):" -ForegroundColor Cyan
    New-JunctionSafe -Link "$projectSkills\key-abstraction-identification" -Target $skillRoot
}
