<#
.SYNOPSIS
  Create a directory junction so Cursor discovers the abd-delivery-lead agent
...
if (-not $ProjectOnly) {
    ...
    New-JunctionSafe -Link "$userSkills\abd-delivery-lead" -Target $agentRoot
}

if (-not $UserOnly) {
    Write-Host "Project-level junction (.cursor/skills/):"
    New-JunctionSafe -Link "$projectSkills\abd-delivery-lead" -Target $agentRoot
}
