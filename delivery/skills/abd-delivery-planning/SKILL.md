...
**A plan is NOT "run all five stages start to finish."** That is the degenerate case for trivial engagements with no risk. Real plans decompose the work into targeted runs that flush out unknowns early.
...
- **Companion file:** Checkbox execution lives in **`docs/planning/delivery-war-room/delivery-plan-checklist.md`** (see **`track_task`**). The narrative plan and the war-room checklist must stay **aligned** when either changes.
...
| --------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Stages**            | Which stages execute, start through end                                       | `shaping → discovery → exploration → specification → engineering` or bug-fix: `engineering` only          |
...
| **Across team members** | Between stages (e.g. after exploration, before specification) | Normal flow, handoff validation                   |
...
This flow has **five bootcamp stages**. Per-stage **entry conditions**, **exit gates**, **team roles**, and **practice skills** by family are in **`../../content/stages/`** — start at **[stages/README.md](../../content/stages/README.md)**.


| #   | Stage            | Default role  | Primary focus | Stage file |
| --- | ---------------- | ------------- | ------------- | ---------- |
| 1   | Shaping          | Product Owner | Outline map, module partition, arch outline | `shaping.md` |
| 2   | Discovery        | Product Owner | Full map, domain terms, UX IA, blueprint; **thin slices last** | `discovery.md` |
| 3   | Exploration      | Product Owner | UL refresh, AC, UX mockup, arch template | `exploration.md` |
...
- **Regressions and test-led fixes** — when the signal is a **failing test**, **wrong behavior**, or a **small defect**, plan **separate steps with distinct intents** (e.g. Exploration to **isolate** scope → Engineering to **prove** the fix → Exploration / Specification to **reverse-engineer** AC/spec from corrected behavior). Same stages can appear in multiple rows when scope differs; intent explains *why* each step exists — see **`strategies/bug-fix.md`**.
...
| --- | -------------------------- | ---------------------------------------------- | ----------------------------------------- | ------------------------ |
| 1   | Discovery                  | Only stories touching CRM, SSO, or billing     | Per-story                                 | Map integration surface  |
...
| 5   | Discovery (re-slice)       | Re-order full map                              | Cross-team                                | Order by value           |
| 6+  | Exploration → Specification → Engineering | Remaining slices               | Per-slice                                 | Routine                  |
...
| Run | Stages                         | Scope                                     | Checkpoint Policy           | Rationale                   |
| --- | ------------------------------ | ----------------------------------------- | --------------------------- | --------------------------- |
| 1   | Discovery                      | Full map; slice by compliance surface     | Per-epic PO                 | Rules are the risk          |
| 2   | Exploration                    | Auditor / Finance slices — heaviest rules | Per-AC; regulatory accuracy | AC = compliance             |
| 3   | Specification                  | Same slice; real VAT/retention values     | Per-scenario                | Concrete values expose gaps |
| 4   | Engineering                    | Compliance slice end-to-end               | Per-test                    | Tests prove compliance      |
| 5+  | Exploration → Specification → Engineering | Other slices              | Per-slice                   | Lower risk                  |
...
| 3   | Discovery (re-slice)           | Simplest E2E slice first          | Per-slice PO                 | Prove domain model                  |
| 4   | Exploration → Engineering      | Basic lifecycle slice             | Per-story                    | Entities in code                    |
| 5   | Exploration → Specification    | Hard algorithmic stories          | Per-AC + user vs transcripts | Validate before engineering         |
| 6+  | Specification → Engineering    | Algorithm stories then rest       | Per-test / per-slice         | Incremental                         |
...
| Run | Stages                     | Scope                          | Checkpoint Policy | Rationale         |
| --- | -------------------------- | ------------------------------ | ----------------- | ----------------- |
| 1   | Discovery                  | Map from existing API + manual | Per-endpoint PO   | Old system = spec |
| 2   | Exploration                | AC matches legacy behavior     | Per-AC vs PHP     | Contract          |
| 3   | Engineering                | Tests against old API; then implement new | Per-test  | Safety net        |
...
4. Sequence of runs; for **each run**, include the items from **§2c** (each **rationale** must state the **concrete outcome**, not only risks). **Multi-increment engagements:** include **every slot** per run — no routine-template or offset shortcuts.

**Persist** the same content to **`<workspace>/docs/planning/abd-delivery-lead/agile-delivery-plan.md`** (see **Where to save the plan**) **before** you stop for confirmation, or immediately after the user confirms if you presented from a draft—so the delivery lead and other agents can **read the plan from disk** on the next session. The persisted file must include the **context inventory** so missing inputs are not lost between sessions.
...
| `rules/*.md`                           | Mechanical rules for plan quality (seven rules, bound to one scanner)    |
| `scanners/plan-shape-scanner.py`       | Single scanner evaluating the seven plan-shape rules against the saved plan |
| `scanners/tests/*.md`                  | Good / bad plan fixtures used to verify the scanner                    |

### Validating a saved plan

After writing or revising `<workspace>/docs/planning/abd-delivery-lead/agile-delivery-plan.md` (Step 2 or Step 7 of the delivery lead), run:

```
python skill-helpers/skills/execute-skill-using-skills-rules/scripts/run_scanners.py \
    --skill-root abd-delivery-planning \
    --workspace <workspace>
```

The runner discovers `scanners/plan-shape-scanner.py` from each rule's `scanner: plan-shape` frontmatter (deduplicated to a single invocation) and from flat discovery under `scanners/`. The scanner reads the plan at `<workspace>/docs/planning/abd-delivery-lead/agile-delivery-plan.md` and reports violations for any of the six rules; exit code 0 = clean, 1 = at least one violation, 2 = path resolution failure. When no plan exists yet the scanner exits 0 silently.

**Engagement workspace (not in this repo):** `docs/planning/abd-delivery-lead/agile-delivery-plan.md` — canonical **saved** agile delivery plan for the engagement; see **Where to save the plan**.