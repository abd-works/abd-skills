/**
 * DomainNameListView.tsx — Container component with search and list rendering.
 *
 * Uses the useDomainNames hook for state and delegates item display
 * to the presentational DomainNameCardView component.
 *
...
import { DomainNameCardView } from './DomainNameCardView';

interface DomainNameListViewProps {
  onSelectItem?: (item: DomainName) => void;
}

export function DomainNameListView({ onSelectItem }: DomainNameListViewProps) {
  ...
  return (
    <div className="domainName-list">
      ...
      />
      {displayed.map(item => (
        <DomainNameCardView key={item.id} item={item} onSelect={onSelectItem} />
      ))}
    </div>
  );
}
