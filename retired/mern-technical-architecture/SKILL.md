...
- **Cross-layer method naming** — same domain verb name in shared, API client, controller, service, and route.
- **Arg name preservation** — argument names are identical across layers; only types narrow.
- **Property casing** — camelCase in TypeScript, snake_case in JSON bodies and MongoDB documents.
- **Standard mutation response** — all POST/PUT/DELETE on the same aggregate return the same snapshot type.
- **Consistent View naming** — React components end with `View`; file names match component names.
