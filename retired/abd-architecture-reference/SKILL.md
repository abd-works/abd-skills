...
`abd-architecture-specification` produces (or assigns) reference sections that specify *how* a mechanism is structured. This skill **assigns** existing reference and code when already present, or **creates** missing reference sections and implementation files.
